...
{
    public class BaseModelRenderProcessor<TModelComponent, URenderMesh> : EntityProcessor<TModelComponent, RenderModel>, IEntityComponentRenderProcessor where TModelComponent : ModelComponent where URenderMesh : RenderMesh, new()
    {
        private Material fallbackMaterial;

        public Dictionary<TModelComponent, RenderModel> RenderModels => ComponentDatas;

        public VisibilityGroup VisibilityGroup { get; set; }

        public BaseModelRenderProcessor() : base(typeof(TransformComponent))
        {
        }
        ...
        /// <inheritdoc />
        protected override RenderModel GenerateComponentData(Entity entity, TModelComponent component)
        {
            return new RenderModel();
        }

        /// <inheritdoc />
        protected override bool IsAssociatedDataValid(Entity entity, TModelComponent component, RenderModel associatedData)
        {
            return true;
        }

        /// <inheritdoc />
        protected override void OnEntityComponentRemoved(Entity entity, TModelComponent component, RenderModel renderModel)
        {
            // Remove old meshes
        ...
        private void UpdateRenderModel(TModelComponent modelComponent, RenderModel renderModel)
        {
        ...
        private void CheckMeshes(TModelComponent modelComponent, RenderModel renderModel)
        {
                ...
                if (model != null)
                {
                    ...
                    if (modelComponent.Enabled)
                    {
                        ...
                        for (int sourceMeshIndex = 0; sourceMeshIndex < model.Meshes.Count; sourceMeshIndex++)
                        {
                            ...
                            {
                                UpdateMaterial(renderModel.Meshes[meshIndex] as URenderMesh, newMaterial?.Passes[pass], model.Materials.GetItemOrNull(materialIndex), modelComponent);
                            }
                        }
                    }
                }
        ...
        RegenerateMeshes:
            ...
            var renderMeshes = new URenderMesh[materialMeshCount];
            for (int sourceMeshIndex = 0; sourceMeshIndex < model.Meshes.Count; sourceMeshIndex++)
            {
                ...
                for (int pass = 0; pass < material.MeshCount; ++pass, ++meshIndex)
                {
                    ...
                    renderMeshes[meshIndex] = new URenderMesh
                    {
